...
def crear_imagen_mapa(num_circulos, divisiones_por_circulo, tipo_mapeo, mostrar_tipos=None, dpi=300):
    ...
    # Ajustar tamaño de figura según DPI para mejor calidad
    if dpi >= 1200:
        figsize = (20, 12)  # Figura más grande para resolución profesional
        title_fontsize = 18
        legend_fontsize = 14
        line_width_multiplier = 1.5
    elif dpi >= 900:
        figsize = (18, 11)  # Figura grande para ultra alta calidad
        title_fontsize = 16
        legend_fontsize = 12
        line_width_multiplier = 1.3
    elif dpi >= 600:
        figsize = (16, 10)  # Figura estándar para alta calidad
        title_fontsize = 14
        legend_fontsize = 11
        line_width_multiplier = 1.1
    else:
        figsize = (16, 10)  # Figura estándar
        title_fontsize = 14
        legend_fontsize = 10
    ...
    # Crear figura con espacio para leyenda (tamaño adaptativo según DPI)
    ...
    fig.suptitle(titulo, fontsize=title_fontsize, fontweight='bold', y=0.95)
    ...
    for tipo, color in colores.items():
        ...
        if puntos:
            ...
            # Tamaño adaptativo basado en logaritmo del número de círculos
            # Fórmula mejorada para mejor escalado
            base_size = max(0.5, 8 - math.log(num_circulos + 1) * 1.5)
            
            if tipo == 'compuesto':
                size = base_size * 0.6  # Compuestos más pequeños
            else:
                size = base_size
                
            # Bonificación para tipos especiales
            if tipo in ['mersenne', 'fermat']:
                size *= 1.3
            elif tipo in ['primo_gemelo', 'sophie_germain']:
                size *= 1.1
    ...
    # Configurar ejes principales con fondo claro
    ax_main.set_aspect('equal')
    ax_main.grid(True, alpha=0.3, color='lightgray', linewidth=0.5)
    ax_main.set_facecolor('#f8f9fa')  # Fondo claro como antes
    ...
    # Dibujar círculos guía más visibles
    for i in range(1, num_circulos + 1):
        # Líneas sutiles sin amarillo (adaptativo según DPI)
        if i % 10 == 0:
            # Círculos principales cada 10 (sin amarillo)
            circle = plt.Circle((0, 0), i, fill=False, color='white', alpha=0.6, 
                              linewidth=1.0 * line_width_multiplier, linestyle='-')
        else:
            # Círculos secundarios
            circle = plt.Circle((0, 0), i, fill=False, color='lightgray', alpha=0.4, 
                              linewidth=0.5 * line_width_multiplier, linestyle='-')
        ax_main.add_patch(circle)
    
    # Agregar líneas radiales para mostrar segmentos (solo si no hay demasiados)
    if divisiones_por_circulo <= 36:
        for i in range(0, divisiones_por_circulo, max(1, divisiones_por_circulo // 12)):
            ...
            # Líneas radiales sutiles sin amarillo (adaptativo según DPI)
            if i % (divisiones_por_circulo // 2) == 0:
                ax_main.plot([0, x_end], [0, y_end], color='white', alpha=0.4, 
                           linewidth=0.8 * line_width_multiplier, linestyle='-')
            else:
                ax_main.plot([0, x_end], [0, y_end], color='lightgray', alpha=0.2, 
                           linewidth=0.3 * line_width_multiplier, linestyle=':')
    
    # Agregar círculo central sutil
    center_circle = plt.Circle((0, 0), 0.2, fill=True, color='white', alpha=0.6)
    ...
    ax_legend.text(0.1, 0.95, 'LEYENDA DE COLORES', fontweight='bold', fontsize=legend_fontsize)
    ...
    for tipo, color in colores.items():
        if mostrar_tipos.get(tipo, True) and contadores.get(tipo, 0) > 0:
            ...
            ax_legend.text(0.2, y_pos, f'{nombre}: {count:,}', 
                          fontsize=legend_fontsize-2, verticalalignment='center')
    ...
    y_pos -= 0.05
    ax_legend.text(0.1, y_pos, 'ESTADÍSTICAS:', fontweight='bold', fontsize=legend_fontsize-1)
    y_pos -= 0.06
    ax_legend.text(0.1, y_pos, f'Primos renderizados: {total_primos:,}', fontsize=legend_fontsize-3)
    y_pos -= 0.04
    ax_legend.text(0.1, y_pos, f'Rango analizado: 1-{limite:,}', fontsize=legend_fontsize-3)
    y_pos -= 0.04
    
    if total_posibles > 0:
        densidad = (total_primos / total_posibles) * 100
        ax_legend.text(0.1, y_pos, f'Densidad primos: {densidad:.1f}%', fontsize=legend_fontsize-3)
